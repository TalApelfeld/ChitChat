...
type CreateTableBody = { room: string; name: string; friend: string };
type GetChatsRooms = { name: string };
...
app.get("/", async (_, res) => {
  const result = (await sql`SELECT version()`) as any;
...
app.post(
  "/rooms",
  async (req: Request<{}, any, CreateTableBody>, res: Response) => {
...
app.post(
  "/chats",
  async (req: Request<{}, any, GetChatsRooms>, res: Response) => {
    const { name } = req.body;
...
app.post(
  "/addGroup",
  async (req: Request<{}, any, CreateGroupBody>, res: Response) => {
...
app.post("/groups", async (req: Request<{}, any, GetGroups>, res: Response) => {
  const { name } = req.body;
  try {
...
io.on("connection", (socket) => {
  ...
  socket.on("client:message:sent", async (msg: IMessage) => {
  ...
  socket.on("client:getMessages", async (room: string) => {
    const messages = await sql`
      SELECT 
        id,
        room_id,
